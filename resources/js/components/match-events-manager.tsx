...
export function MatchEventsManager({
	...
	const managingTeamId = isHomeLeader ? match.home_team_id : match.away_team_id;
	const managingTeamName = isHomeLeader ? match.home_team.name : match.away_team?.name;
	...
	const renderEventsList = (teamEvents: MatchEvent[], teamName: string, canDelete: boolean) => {
	...
	return (
		<Card>
			<CardHeader className="pb-3">
				<div className="flex items-center justify-between">
					<div>
						...
						<CardDescription>Record all goals scored during the match</CardDescription>
					</div>
								...
								<form onSubmit={handleSubmit}>
									...
									<DialogFooter>
										...
										<Button type="submit" disabled={processing}>
											{processing ? "Recording..." : "Record Goal"}
										</Button>
									</DialogFooter>
								</form>
							</DialogContent>
			...
			<CardContent className="space-y-4">
				<div>
					<h4 className="text-xs font-semibold mb-2">{match.home_team.name}</h4>
					{renderEventsList(homeEvents, match.home_team.name, isHomeLeader)}
				</div>
				{match.away_team && (
					<div>
						...
						{renderEventsList(awayEvents, match.away_team.name, isAwayLeader)}
					</div>
				)}
			</CardContent>
		</Card>
