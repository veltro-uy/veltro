...
export default function Show({ team, isMember, canManage }: Props) {
	...
	const maxMembers = team.max_members ?? getMaxMembersForVariant(team.variant);
			...
			<div className="flex h-full flex-1 flex-col gap-6 p-6">
				...
				<div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
					<div className="flex gap-4">
						<TeamAvatar name={team.name} logoUrl={team.logo_url} size="xl" />
						<div>
					...
					<div className="flex gap-2">
						...
						{!isMember && !isFull && (
							<JoinRequestDialog teamId={team.id} teamName={team.name} />
						)}
						{!isMember && isFull && (
							<Button disabled>
				...
				<div className="grid gap-6 lg:grid-cols-3">
								...
								<CardDescription>
									<div className="flex items-center gap-2">
										<span className={capacityColor}>
											{currentMembers}/{maxMembers} miembros
										</span>
										{isFull && (
					...
					<div className="space-y-6 lg:col-span-2 lg:order-1">
						...
						{canManage && team.pending_join_requests?.length > 0 && (
							<Card>
								<CardHeader>
									<CardTitle>Solicitudes de Unión Pendientes</CardTitle>
									<CardDescription>
										Revisa y responde a las solicitudes de unión
									</CardDescription>
								</CardHeader>
								<CardContent>
									<div className="space-y-3">
										{team.pending_join_requests.map((request) => (
											<div
												key={request.id}
				...
				<AlertDialog open={showLeaveDialog} onOpenChange={setShowLeaveDialog}>
					<AlertDialogContent>
						<AlertDialogHeader>
							<AlertDialogTitle>Abandonar Equipo</AlertDialogTitle>
							<AlertDialogDescription>
								¿Estás seguro de que quieres abandonar{" "}
								<span className="font-semibold">{team.name}</span>? Necesitarás
								solicitar unirte nuevamente si cambias de opinión.
							</AlertDialogDescription>
						</AlertDialogHeader>
						<AlertDialogFooter>
